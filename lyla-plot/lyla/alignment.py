...
class Colorbox:
    ...
    def plot( self, ax, heatmap ):
        BIN= 0.2
        cell_width = (self.width / ( heatmap.max_identity - heatmap.min_identity ))*BIN
        ...
        if heatmap.max_identity - heatmap.min_identity >= 30:
            SCALE = 10
        elif heatmap.max_identity - heatmap.min_identity >= 15:
            SCALE = 5
        elif heatmap.max_identity - heatmap.min_identity > 5:
            SCALE = 2
        else:
            SCALE = 1
        for i in range( int(heatmap.min_identity/BIN), int(heatmap.max_identity/BIN) + 1 ):
            align_color=heatmap.convert_identity2color( i*BIN )
            ...
            if( i*BIN % SCALE == 0 ):
                num_legend = common.Text( int(i*BIN), 6, cell_originx, self.origin_y - 0.06, 'center', 'top' )
...
def plot_alignment4original( seqs, ax, heatmap, size, fn, args ):
    with open( fn , 'r' ) as file:
        for line in file:
            buf = line.rstrip( '\n' ).split( '\t' )
            if( float( buf[6] ) < args.min_identity ):
                continue
            if( int( buf[2] ) - int( buf[1] ) + 1 < args.min_alignment_len ):
                continue
...
def plot_alignment4blastn( seqs, ax, heatmap, size, fn, args ):
    with open( fn , 'r' ) as file:
        for line in file:
            buf = line.rstrip( '\n' ).split( '\t' )
            if( float( buf[2] ) < args.min_identity ):
                continue
            if( int( buf[7] ) - int( buf[6] ) + 1 < args.min_alignment_len ):
                continue
...
def plot_alignment4lastz( seqs, ax, heatmap, size, fn, args ):
    with open( fn , 'r' ) as file:
        for line in file:
            if( line[0:1] == "#" ):
                continue
            buf = line.rstrip( '\n' ).split( '\t' )
            if( float( buf[12][:-1] ) < args.min_identity ):
                continue
            if( int( buf[5] ) - int( buf[4] ) + 1 < args.min_alignment_len ):
                continue
...
def plot_alignment4mummer( seqs, ax, heatmap, size, fn, args ):
    with open( fn , 'r' ) as file:
        for line in file:
            buf = line.rstrip( '\n' ).split( )
            if( float( buf[9] ) < args.min_identity ):
                continue
            if( int( buf[1] ) - int( buf[0] ) + 1 < args.min_alignment_len ):
                continue
